...
func TestFailParseFromYaml(t *testing.T) {
	tcs := []struct {
	...
	}{
		{
			...
			foo: bar
			`,
			err: "error unmarshaling source: unable to parse source \"my-mindsdb-instance\" as \"mindsdb\": [2:1] unknown field \"foo\"\n   1 | database: my_db\n>  2 | foo: bar\n       ^\n   3 | host: 0.0.0.0\n   4 | name: my-mindsdb-instance\n   5 | password: my_pass\n   6 | ",
		},
		{
			...
			password: my_pass
			`,
			err: "error unmarshaling source: unable to parse source \"my-mindsdb-instance\" as \"mindsdb\": Key: 'Config.Host' Error:Field validation for 'Host' failed on the 'required' tag",
		},
	}
	for _, tc := range tcs {
		t.Run(tc.desc, func(t *testing.T) {
