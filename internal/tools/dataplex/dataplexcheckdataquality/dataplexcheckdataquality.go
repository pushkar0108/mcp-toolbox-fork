...
package dataplexcheckdataquality
...
func (cfg Config) Initialize(context.Context) (tools.Tool, error) {
	...
	specJSON := parameters.NewStringParameter("specJSON", "A JSON-serialized string of the Dataplex DataQualitySpec proto message containing the rules and optional configuration. Example format: {\"rules\": [{\"column\": \"my_col\", \"dimension\": \"COMPLETENESS\", \"nonNullExpectation\": {}}], \"catalogPublishingEnabled\": false}")
	publish := parameters.NewBooleanParameter("publish", "Whether to publish the generated quality scores/results to the Dataplex Universal Catalog.")

	allParameters := parameters.Parameters{resourcePath, location, specJSON, publish}
...
func (t Tool) Invoke(ctx context.Context, primitiveMgr tools.SourceProvider, params parameters.ParamValues, accessToken tools.AccessToken) (any, util.ToolboxError) {
	...
	specJSON, _ := paramsMap["specJSON"].(string)
